function [route,numExpanded] = AStarGrid1 (input_map, start_coords, dest_coords)
...
%     %
    ...
    if j ~= 1
        if map(i, j-1) ~= 2 && (map(i, j-1) ~= 3 && map(i, j-1) ~= 5)
            %t = distanceFromStart(current)+1;
            if g(i,j-1)>g(current)+1
                g(i,j-1)=g(current)+1;

                if i==nrows
                    f(i,j-1)=g(i,j-1)+H(i-1,j-1)
                else
